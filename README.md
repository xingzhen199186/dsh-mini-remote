...
两条加密路各有各的前提，说清楚免得你走错：这一条用的是形如 `你的电脑名.你的网络名.ts.net` 的域名，**它只在你的 Tailscale 网络内部解析得出**，所以手机上得开着 Tailscale 的「使用 Tailscale DNS 设置」——手机上要是报「找不到服务器」，就是这里没开。隧道那条用的是 Cloudflare 的域名，谁都能解析，不挑手机设置，代价是地址每次重启都会换。

...
export async function createMiniServer({ store, config, token, bindAddresses, tunnelHost, onInstruction, onStop, onUnqueue, onUpload, log, tree, browse, build }) {
  ...
  async function handle(req, res) {
    ...
    // 【临时诊断，查完就撤】给一个会话 id，把「问 DSH 要模型」每一步的实际情况
    // 原样倒出来。用 POST 传 id 是因为这条路径上读请求体的写法已经现成。
    if (req.method === 'POST' && path === '/mini/api/_probe') {
      const body = await readBody(req)
      // 先问**真正在用的那个方法**——要查的是整条路通不通，不是零件齐不齐。
      const model = nav.sessionModel ? await nav.sessionModel(body?.sessionId) : '没有这个方法'
      const probe = nav.debugSession ? await nav.debugSession(body?.sessionId) : '没有这个方法'
      sendJson(res, 200, { ok: true, model, probe })
      return
    }

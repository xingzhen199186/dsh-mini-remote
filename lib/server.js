...
import { renderPage, readArt } from './page.js'
 ...
 */
export async function createMiniServer({ store, config, token, bindAddresses, tunnelHost, onInstruction, onStop, onUnqueue, onUpload, log, tree, build }) {
  /** @type {Set<import('node:http').ServerResponse>} */
  const clients = new Set()

  // 没传 tree（测试里就是这样，headless 组合里也拿不到那两个服务）时给个空壳，
  // 导航栏显示「取不到」，遥控本身照常。
  const nav = tree ?? { listWorkspaces: async () => [], listSessionsOf: async () => null }
  ...
  async function handle(req, res) {
    ...
    if (req.method === 'GET' && path === '/mini/api/version') {
      sendJson(res, 200, { ok: true, build: build ?? 'unknown' })
      return
    }
    ...
    if (req.method === 'GET' && path === '/mini/api/workspaces') {
      ...
      const workspaces = await nav.listWorkspaces(runningIds, url.searchParams.get('refresh') === '1')

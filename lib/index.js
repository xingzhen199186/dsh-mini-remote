...
export function apply(ctx, config = {}) {
  ...
  // ------------------------------------------------------------------
  // 挂在根上这一版**加上「插到队首」**（`{ prepend: true }`）。
  //
  // 为什么：这个瀑布是**遇到第一个愿意接的人就停**，而电脑上的浏览器（经由
  // `dsh-api-remotes`）本来就接这个提问——排在它后面就永远轮不到我。
  // 插到队首之后，我可以自己决定这次接不接；不接就 `next()` 让给浏览器，它照样能答。
  // **插队不是为了抢，是为了有得选。**
  ctx.on('user-questions/request', function (request, next) {
    try {
      console.log('[dsh-mini-remote] 【根上】提问钩子被叫到：' + JSON.stringify({
        agentId: request?.agent?.id ?? null,
        questionCount: Array.isArray(request?.questions) ? request.questions.length : null,
      }))
    } catch (err) {
      console.log('[dsh-mini-remote] 【根上】钩子记录失败：' + String(err))
    }
    return next()
  }, { prepend: true })

  const hookedAgents = new Set()
  const hookAgents = () => {
    try {
      const agents = ctx.get('agents')
      const roots = typeof agents?.roots === 'function' ? agents.roots() : []
      for (const agent of roots) {
        if (!agent?.ctx || !agent.id || hookedAgents.has(agent.id)) continue
        hookedAgents.add(agent.id)
        agent.ctx.on('user-questions/request', function (request, next) {
          try {
            console.log('[dsh-mini-remote] 【Agent 范围内】提问钩子被叫到：' + JSON.stringify({
              agentId: request?.agent?.id ?? null,
              questionCount: Array.isArray(request?.questions) ? request.questions.length : null,
            ...
            console.log('[dsh-mini-remote] 【Agent 范围内】钩子记录失败：' + String(err))
          }
          return next()
        }, { prepend: true })
        console.log('[dsh-mini-remote] 已在 Agent ' + agent.id + ' 的范围内挂上提问钩子')
      }
    } catch (err) {
      console.log('[dsh-mini-remote] 补挂提问钩子出错：' + String(err))
    }
  }
  hookAgents()
  const hookTimer = setInterval(hookAgents, 3000)
  ctx.effect(() => () => clearInterval(hookTimer))

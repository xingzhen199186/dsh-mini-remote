...
export function apply(ctx, config = {}) {
  ...
  // ------------------------------------------------------------------
  // 【临时探针，查完就撤】Agent 提问时，本插件到底会不会被叫到？
  //
  // 为什么要先证明这个：那个信号是**限定在提问的那个 Agent 范围内**发出去的
  // （`ask()` 里走的是 `scopeTarget(agent, agent)`），而本插件挂在根上。
  // 「范围不匹配就收不到」是可能的，**不试不知道**。上一块功能就是没先验、
  // 白跑一轮，这次先花一轮把路探明。
  //
  // 只记一笔就让给别人（`next()`），**不改任何行为**——先确认能挂上，再谈接管。
  // ------------------------------------------------------------------
  ctx.on('user-questions/request', function (request, next) {
    try {
      console.log('[dsh-mini-remote] 提问钩子被叫到：' + JSON.stringify({
        hasAgent: Boolean(request?.agent),

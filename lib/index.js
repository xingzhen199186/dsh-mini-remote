...
export function apply(ctx, config = {}) {
  ...
  const nav = {
    ...
    sessionModel: async (sessionId) => {
      ...
      if (typeof q?.listEvents !== 'function') return { ok: false, reason: 'unavailable' }
      let events
      try {
        events = await q.listEvents(sessionId)
      } catch (err) {
        return { ok: false, reason: 'read-failed', error: String(err && err.message ? err.message : err) }
      }
      if (!Array.isArray(events)) return { ok: false, reason: 'read-failed' }
      for (let i = events.length - 1; i >= 0; i--) {
        const e = events[i]
        if (!e || e.type !== 'request/header') continue
        const cfg = e.data?.header?.config
        if (cfg && typeof cfg.model === 'string') return { ok: true, model: shape(cfg) }
      }

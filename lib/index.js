...
export function apply(ctx, config = {}) {
  ...
  ctx.on('user-questions/request', function (request, next) {
    ...
    return Promise.resolve(mini.server).then((server) => {
      // 【临时仪表，查完就撤】照亮最后一步：服务拿到手之后，"手机在不在"这个判据说了什么。
      const 手机在不在 = server && typeof server.hasPhone === 'function' ? server.hasPhone() : '问不了'
      console.log('[dsh-mini-remote] 服务到手 → 手机在不在=' + 手机在不在 +

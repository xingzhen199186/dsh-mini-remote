...
import { listWorkspaces, listSessionsOf, createSessionIn, createWorkspaceAt, sessionScale } from './tree.js'
...
export function apply(ctx, config = {}) {
  ...
  /**
   * 整份读的两条红线（2026-09-25 事故后加的护栏）。
   *
   * 事故：手机绑定的会话是**带 310 个子会话的父会话**（26MB 压缩 / 2.2 万事件），
   * 开机预热历史时 `readSession` 触发 v3→v4 迁移，把每个子会话整份解出来，
   * 默认约 4GB 堆 20 秒 OOM。这两条阈值是「手机上宁可少看一点，也别把 DSH 拖死」
   * 的取舍，都写在这儿方便调。
   */
  const HISTORY_MAX_CHILDREN = 20
  const HISTORY_MAX_BYTES = 8 * 1024 * 1024

  /**
   * 手机端读历史的**上限**（2026-09-25 用户要求：读历史要有上限）。
   *
   * 上面两条红线决定「要不要绕开内核那条整份读」，这里三条决定
   * 「自己读的时候最多读多少、最多给手机多少」。三层**都跟会话总量无关**：
   *   - 窗口：只读日志末尾这么多**压缩**字节；
   *   - 事件：窗口里最多留多少条事件（再向前对齐到一条 user 消息，让重放从一轮的开头起）；
   *   - 条目：重放成聊天记录后，最多给手机多少条（≈ 轮数 × 2）。
   */
  // 窗口按「本机最坏的那条日志」实测选的（26MB / 2.2 万事件）：16MB 窗口 → 约 1.2 万事件、
  // 197 条聊天记录、472ms、堆峰 ~400MB；4MB 窗口只有 29 条（太少）。事件上限只作极端兜底。
  ...
  async function readHistory(sessionId) {
    ...
    try {
      ...
      return {
        ...
        note: capped.dropped > 0 ? `只显示最近 ${HISTORY_MAX_ENTRIES} 条` : null,
      }
    } catch (err) {
   ...
   *
   * 什么时候用它：会话「大」的时候。那条整份读的路会把整份日志连同**所有直接子会话**
   * 解出来（v3→v4 迁移要它们），代价一旦付出就收不回来——所以宁可少读一点。
  ...
  function readTailHistory(sessionId) {
    ...
    return {
      entries: capped.entries,
      latest: lastReply(capped.entries, sessionId),
      // 老实说清「读到的是不是全部」：窗口把整份都盖住了就不该再说「不是全部」。
      truncated: tail.truncated,
      note: tail.truncated
        ? `这个会话很大，只显示了最近 ${capped.entries.length} 条（读取上限：${HISTORY_MAX_ENTRIES} 条 / ${windowMb}MB 窗口）`
        : null,
    }
  }
  ...
  async function ensureHistory(sessionId) {
    ...
    // 不能等下面那次异步量规模回来（否则刚绑定那一下会显示成空会话）。
    historyPending.add(sessionId)

    // 读之前先量规模：内核那条整份读会把子会话一起解出来，代价收不回来。
    // 「大」→ 走有上限的尾部读；「小」→ 仍走内核的完整读（日志权威，会话活着还免读盘）。
    const scale = await sessionScale({ query: treeServices.query, sessionId })
    const tooBigForWholeRead =
      scale.children > HISTORY_MAX_CHILDREN || (scale.bytes ?? 0) > HISTORY_MAX_BYTES
    let got = null
    try {
      if (tooBigForWholeRead) {
        const size = scale.bytes == null ? '未知' : (scale.bytes / 1048576).toFixed(1) + 'MB'
        log.info(`规模上限：${sessionId} 改走尾部读（子会话 ${scale.children}，日志 ${size}）`)
      ...
      historyCache.set(sessionId, {
        ...
        note:
          got?.note ??
          (tooBigForWholeRead ? '这个会话很大，最近这一段也没读出来' : null),
      })
  ...
  ctx.on('user-questions/request', function (request, next) {
    ...
    if (!agentId || agentId !== bound || questions.length === 0) return next()
    if (!phoneReady) return next()
    if (typeof miniControl.askPhone !== 'function') return next()
    // ——— 同步交出"将来会有答案"的东西；下面的等待都在这里，不再有同步判断 ———
    return miniControl.askPhone(questions, agentId).then((answer) => {

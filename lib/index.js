...
export function apply(ctx, config = {}) {
  ...
  // ------------------------------------------------------------------
  // 【临时探针 2，查完就撤】Agent 提问时，本插件到底会不会被叫到？
  //
  // **第一版挂在根上，实测没被叫到**：日志确认是活的（末尾就是这个版本的启动输出），
  // 而提问之后文件一分钟都没再动过。原因推断：`ask()` 对带 Agent 的提问走的是
  // `scopeTarget(agent, agent)`，也就是**限定在那个 Agent 的范围内**发出去——
  // 挂在外面的收不到。
  //
  // 所以这一版改成：对每个活着的根 Agent，**在它自己的范围内**挂一个监听。
  // Agent 是来来去去的，所以每 3 秒补挂一次，挂过的记下来不重复。
  //
  // 仍然只记一笔就让路（`next()`），**不改任何行为**。
  // ------------------------------------------------------------------
  ...
  //    不是倒计时，是「手机还在不在」。
  //
  // 服务实例晚于这里创建，所以先放一个可变的持有对象、被叫到时现读（和 treeServices 同理）。
  // ------------------------------------------------------------------
  const mini = { server: null }
  ...
  ctx.on('user-questions/request', function (request, next) {
    ...
    // 【临时仪表，查完就撤】无条件记一笔——上一版只在出错时记，导致我把"没有证据"当成了"证据"。
    console.log('[dsh-mini-remote] 提问钩子被叫到 → 服务=' + (mini.server ? '已有凭据' : '还没有') +
      ' | 问的=' + agentId + ' | 绑的=' + bound + ' | 题目数=' + questions.length)
    // ——— 同步阶段：只做"接不接"的判断，绝不 await ———
    if (!mini.server || !agentId || agentId !== bound || questions.length === 0) return next()
    // ——— 同步交出"将来会有答案"的东西；下面的等待都在这里，不再有同步判断 ———
    return Promise.resolve(mini.server).then((server) => {
      // 【临时仪表，查完就撤】把等来的东西**原样打出来**——光问"是不是函数"不够，
      // 得看它到底是什么、身上有哪些字段，才能知道我把什么当成了服务。
      console.log('[dsh-mini-remote] 服务到手 → 类型=' + Object.prototype.toString.call(server) +
        ' | 构造器=' + (server && server.constructor ? server.constructor.name : '无') +
        ' | 字段=' + (server && typeof server === 'object' ? Object.keys(server).join(',') : '不是对象') +
        ' | mini.server 本身=' + Object.prototype.toString.call(mini.server) +
        ' | 有 askPhone=' + Boolean(server && typeof server.askPhone === 'function'))
      if (!server || typeof server.askPhone !== 'function') return next()
      if (!server.hasPhone()) return next()
      return server.askPhone(questions, agentId).then((answer) => {

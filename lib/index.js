...
export function apply(ctx, config = {}) {
  ...
  ctx.on('user-questions/request', async function (request, next) {
    // 【临时仪表，查完就撤】把每个判断点都记一笔。
    //
    // 为什么非装不可：这三个条件是一票否决式的，任何一条不过就**静默让路**。而探针版
    // 是无条件记账的，真版本只在出错时记账——所以"日志里没有"**根本推不出"没被叫到"**。
    // 我上一轮就是拿"没有证据"当成了"证据"，又栽在同一个坑里。装上仪表，卡在哪一条
    // 就是日志直接说的，不用猜。
    const 仪表 = []
    try {
      // `createMiniServer` 是**异步**函数：它返回的不是服务器本身，而是「将来会给你
      // 服务器」的凭据。这里用 `await Promise.resolve(...)` 两种都接住——已经是服务器就
      // 直接用，还是凭据就等它好了再用。没起来时这里是 null，下面 `!server` 会让给电脑。
      const server = await Promise.resolve(mini.server)
      const agentId = request?.agent?.id ?? null
      const bound = store.snapshot().boundSessionId
      const questions = Array.isArray(request?.questions) ? request.questions : []
      仪表.push('服务=' + (server ? '有' : '无'))
      仪表.push('问的=' + agentId)
      仪表.push('绑的=' + bound)
      仪表.push('题目数=' + questions.length)
      仪表.push('手机连接数=' + (server && typeof server.hasPhone === 'function' ? (server.hasPhone() ? '有' : '无') : '问不了'))
      console.log('[dsh-mini-remote] 提问钩子被叫到 → ' + 仪表.join(' | '))
      // 三个条件缺一不可：手机连着、问的正是手机上绑着的那个会话、提问里真有内容。
      if (!server || !agentId || agentId !== bound || questions.length === 0) return next()
      if (!server.hasPhone()) return next()
      const answer = await server.askPhone(questions, agentId)
      ...
      console.log('[dsh-mini-remote] 接管提问出错，让回电脑：' + String(err) + '　仪表：' + 仪表.join(' | '))
      return next()
    }
  }, { prepend: true })

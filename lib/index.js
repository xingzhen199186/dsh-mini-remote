...
export function apply(ctx, config = {}) {
  ...
  ctx.on('user-questions/request', function (request, next) {
    ...
    const 手机在不在 = typeof miniControl.hasPhone === 'function' ? miniControl.hasPhone() : '问不了'
    console.log('[dsh-mini-remote] 提问钩子 → 手机在不在=' + 手机在不在 +
      ' | 有 askPhone=' + (typeof miniControl.askPhone === 'function'))
    if (!agentId || agentId !== bound || questions.length === 0) return next()
    if (typeof miniControl.hasPhone !== 'function' || !miniControl.hasPhone()) return next()

...
export function createStore({ file, maxHistory = 50, maxSessions = 12 }) {
   ...
   * ① 硬盘那份还没读出来（刚点进去的那一下）；② 刚发生、日志里还没有的那一两条。
  ...
  function mergeHistory(live, disk) {
    ...
    const last = disk[disk.length - 1].timestamp ?? 0
    const tail = live.filter((m) => (m.timestamp ?? 0) > last)
    return tail.length ? disk.concat(tail) : disk
  }

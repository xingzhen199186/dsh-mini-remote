...
const HINTS = {
  ...
  // 2026-09-25 补一句「它也是加密的」。原来只说了「不用装任何东西」，可**加密这件事
  // 有两条来源，只有 Tailscale 那条依赖手机上的 MagicDNS**——公网隧道用的是
  // Cloudflare 的域名，谁都能解析。所以手机上 Tailscale 的 DNS 设置没开的用户，
  // 想要加密连接应该走这一条，而原来的说明里完全没告诉他这一点。
  public: '不用装任何东西，出门在外也能用，而且它也是加密的——浏览器肯不肯给用通知和'
    + '麦克风看的就是这个。刚打开时等半分钟再扫；地址每次重启 DSH 会换',
  ...
  // 只在 tailnet 内部解析得出，靠的是客户端上的 MagicDNS。
  //
  // **而且有些客户端根本没有 MagicDNS。** 用户手机上装的是 HarmonyOS 的第三方
  // 移植版 MeshArc（flypigJ/Tailscale-OHOS），它自己的说明里写着
  // 「MagicDNS is not enabled; use Tailscale IP addresses」——那类手机上这条路
  // 走不通，**不是设置没开的问题**。所以别写「去开手机上那个开关」：那个开关在
  // 这些客户端上不存在，等于让用户去点一个没有的东西。（第一版就是这么写的，错。）
  //
  // 退路有两条，都是真的：
  //   1. 上面那条明文的 Tailscale 用的是 IP，不需要任何解析，任何情况下都连得上；
  //   2. 公网隧道那条**也是加密的**，用的是 Cloudflare 的域名，谁都能解析。
  // 这是「几条码为什么并存」最实在的答案：它们不是互相替代，是各有各的前提。
  // 2026-09-25 用户实测走通了：他手机上装的正是 MeshArc，**换成官方客户端之后
  // 那条加密地址就打开了**。所以这一句要给出真正的解法——换客户端，而不是绕开。
  // 原来只写了「用不了就换别的路」，那是在绕，不是在解决。
  'tailscale-https': '出门在外也能用，而且连接是加密的——手机浏览器只在加密连接上才肯给用'
    + '通知和麦克风。手机上的 Tailscale 得能解析你这个网络的域名：HarmonyOS 上的第三方'
    + '客户端（MeshArc）不支持这一点，换成官方客户端就能用。实在换不了，就退到上面那条'
    + '明文的（它用 IP，不需要解析），或者开公网隧道那条（它也是加密的，不挑手机）',
}

...
function run(args, timeout) {
  return new Promise((resolve) => {
    execFile(CLI, args, { timeout, encoding: 'utf8' }, (err, stdout, stderr) => {
      if (!err) {
      ...
      resolve({
        ok: false,
        out: `${stdout ?? ''}${stderr ?? ''}`,
        // ENOENT = 这台电脑根本没装 tailscale。别的错（超时、非零退出）都算「装了但没成」。
...
export async function serveEnable(port) {
  const r = await run(['serve', '--bg', String(port)], ACTION_TIMEOUT_MS)
  if (!r.ok) return { ok: false, ...classifyEnableFailure(r.out, r.missing) }
...
export async function serveDisable() {
  const r = await run(['serve', '--https=443', 'off'], ACTION_TIMEOUT_MS)

...
import { foldCommandEvent } from './commands.js'

/**
 * @param {Array<{type:string, time?:number, data?:any}>} events DSH 的原始事件，**按发生顺序**。
 * @returns {Array<{role:'user'|'assistant'|'command', text?:string|null, timestamp:number|null,
 *                  id?:string|null, reason?:string, interrupted?:boolean,
 *                  commandId?:string, name?:string, args?:string, kind?:string}>}
 */
export function replayHistory(events) {
  const tracker = createTurnTracker()
  const out = []
  for (const event of events ?? []) {
    // 斜杠指令（`command/run` / `command/done`）不是「一轮」，提取器不认它——
    // 可它确实写在会话记录里，电脑端也确实画在正文里。用和实时那条路**同一个**
    // 折叠函数（lib/commands.js）接住，两边才不会一个显示「执行中」、一个显示结果。
    if (foldCommandEvent(out, event)) continue
